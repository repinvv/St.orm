        ...
        public void GenerateMethod(Model model, IStringGenerator stringGenerator)
        {
            ...
            stringGenerator.Braces(() =>
            {
                ...
                for (i = 0; i < fields.Count; i++)
                {
                    ...
                    stringGenerator.AppendLine($"parameters.Add(new SqlParameter(\"@parm{i + 1}i\" + i, entity.{fields[i].Name + dbnull}));");
                }
            });
            stringGenerator.AppendLine();
            stringGenerator.AppendLine("i = 0;");
            var matLine = materializerLineGenerator.GenerateMaterializerLine("r", idField.Type, 0);
            stringGenerator.AppendLine("AdoCommands.RunCommand(sb.ToString(), parameters.ToArray(), connection, transaction, " +
                                       $"r => entities[i++].{idField.Name} = {matLine});");
        }
    }
}
